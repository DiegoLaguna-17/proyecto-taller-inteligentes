...
> Formato de ficha basado en el modelo "El Puente de Métricas": cada historia incluye una métrica **técnica** (rendimiento del sistema/modelo), una métrica de **negocio/KPI** (impacto real en pacientes, médicos o la operación), y un **criterio de éxito/restricción** con enfoque *human-in-the-loop* cuando aplica.
...
| Técnica | Negocio |
|---|---|
| Tasa de autenticación exitosa con credenciales válidas > **99%**; 100% de las conexiones cifradas vía HTTPS/TLS 1.2+; 0 contraseñas almacenadas o logueadas en texto plano. | Reducir a **0 incidentes por trimestre** de acceso no autorizado a información médica sensible (cumplimiento de protección de datos de salud). |
...
| Técnica | Negocio |
|---|---|
| Precisión del clasificador multiclase (Normal / Riesgo Hipo / Riesgo Hiper) > **90%**; Tasa de Falsos Negativos < **5%**; Latencia de inferencia p95 < **2000 ms** (definida en la historia original). | Reducir el tiempo de detección de un evento de riesgo glucémico frente al monitoreo manual/periódico en **90%**. |

**Historia de Usuario (Inferencia)**
Como Sistema, quiero evaluar cada nueva toma de glucosa con un modelo de ML en menos de 2 segundos, para clasificarla automáticamente como Normal, Riesgo Hipo o Riesgo Hiper.
...
**Criterio de Éxito / Restricción (Human-in-the-loop)**
Si el servicio de mensajería en la nube (ej. Firebase Cloud Messaging) falla o no confirma la entrega en **2 segundos**, el sistema debe activar un canal de respaldo (ej. SMS o email) en lugar de asumir silenciosamente que la alerta llegó — ningún evento crítico debe depender de un solo canal de notificación.
...
- Los valores numéricos (%, ms, tiempos) son **estimaciones iniciales inferidas** a partir del contexto del proyecto y de los criterios de aceptación ya definidos en cada historia de usuario; deben ajustarse una vez exista una línea base (baseline) real medida sobre el dataset y el pipeline (ver Tarea 15-16 del PLAN_PROYECTO.md).
- Todas las métricas de negocio están formuladas evitando la "Miopía de Precisión": no se limitan a exactitud del modelo, sino a impacto verificable (tiempo, adherencia, carga administrativa, intervención médica).
- Los criterios de éxito de US-01, US-02, US-04, US-06 y US-08 incorporan explícitamente una restricción *human-in-the-loop*, evitando que el sistema tome decisiones críticas de forma autónoma sin supervisión humana.
